...
lab.experiment('Process IMTD Thresholds', () => {
  lab.experiment('Post processing is set to "false"', () => {
    lab.test('should return thresholds as is', async () => {
      const thresholds = processImtdThresholds({ alert: 1.1, warning: 2.1 }, 0, 0, false)

      expectThresholds(thresholds, '2.10', '1.10')
    })

    lab.test('should not be return single null thresholds', async () => {
    ...
    lab.test('then both null thresholds should not return double null thresholds', async () => {
      const thresholds = processImtdThresholds({ alert: null, warning: null }, 0, 0, false)

      expect(thresholds).to.equal([])
    })

    lab.test('should not cause an error when thresholds are empty', async () => {
  ...
  lab.experiment('Post processing is set to "true"', () => {
    lab.test('should return thresholds as is when "stageDatum" is zero', async () => {
      const thresholds = processImtdThresholds({ alert: 1.1, warning: 2.1 }, 0, 0, true)

      expectThresholds(thresholds, '2.10', '1.10')
    })

    lab.test('should return thresholds as adjusted when "stageDatum" is greater than zero', async () => {
      const thresholds = processImtdThresholds({ alert: 1.1, warning: 2.1 }, 2.2, 0, true)

      expectThresholds(thresholds, '-0.10', '-1.10')
    })

    lab.test('should return thresholds as is when "stageDatum" is less than zero', async () => {
      const thresholds = processImtdThresholds({ alert: 1.1, warning: 2.1 }, -2.2, 0, true)

      expectThresholds(thresholds, '2.10', '1.10')
    })

    lab.test('should return thresholds as adjusted when "stageDatum" is less than zero and "stationSubtract" is greater than zero', async () => {
      const thresholds = processImtdThresholds({ alert: 1.1, warning: 2.1 }, -2.2, 0.5, true)

      expectThresholds(thresholds, '1.60', '0.60')
    })

    lab.test('should return thresholds as is when "stageDatum" is less than zero and "stationSubtract" is less than zero', async () => {
      const thresholds = processImtdThresholds({ alert: 1.1, warning: 2.1 }, -2.2, -0.5, true)

      expectThresholds(thresholds, '2.10', '1.10')
    })

    lab.test('should return thresholds as is when "stageDatum" is equal to zero and "stationSubtract" is less than zero', async () => {
      const thresholds = processImtdThresholds({ alert: 1.1, warning: 2.1 }, 0, -0.5, true)

      expectThresholds(thresholds, '2.10', '1.10')
    })

    lab.test('should return thresholds as adjusted when "stageDatum" is equal to zero and "stationSubtract" is greater than than zero', async () => {
      const thresholds = processImtdThresholds({ alert: 1.1, warning: 2.1 }, 0, 0.5, true)

      expectThresholds(thresholds, '1.60', '0.60')
    })

    lab.test('should return thresholds as is when "stageDatum" is equal to zero and "stationSubtract" is zero', async () => {
      const thresholds = processImtdThresholds({ alert: 1.1, warning: 2.1 }, 0, 0, true)
