...
lab.experiment('Target-area tests', () => {
  ...
  lab.test('Check flood severity banner link for Flood alert', async () => {
    ...
    Code.expect(response.payload).to.contain('The River Derwent level upstream at Derby St Marys was <span data-item-value>0.54</span> metres. Property flooding is possible when it goes above 3.30 metres.')
  ...
  lab.test('Check flood severity banner link for Flood warning and multi latest levels', async () => {
    ...
    Code.expect(response.payload).to.contain('<p>The River Pinn level upstream at Avenue Road was <span data-item-value>0.18</span> metres. Property flooding is possible when it goes above 1.46 metres.')
    Code.expect(response.payload).to.contain('<a href="/station/7173?tid=1747543">Monitor the River Pinn level upstream at Avenue Road.</a>')
    Code.expect(response.payload).to.contain('<p>The River Pinn level upstream at Moss Close was <span data-item-value>0.13</span> metres. Property flooding is possible when it goes above 1.15 metres.')
    Code.expect(response.payload).to.contain('<a href="/station/7201?tid=1747541">Monitor the River Pinn level upstream at Moss Close.</a>')
  })
  ...
  lab.test('Displays latest level for a single active but offline station', async () => {
    ...
    Code.expect(response.payload).to.contain('<h2 class="defra-live__title">Latest level</h2>')
    Code.expect(response.payload).to.contain('<p>The River Pinn level upstream at Avenue Road is currently unavailable.</p>')
  ...
  lab.test('Displays latest level for a single suspended station', async () => {
    ...
    Code.expect(response.payload).to.contain('<h2 class="defra-live__title">Latest level</h2>')
    Code.expect(response.payload).to.contain('<p>The River Pinn level upstream at Moss Close is currently unavailable.</p>')
    Code.expect(response.payload).to.contain('<p class="defra-live__supplementary">This level will update automatically</p>')
  })
  ...
  lab.test('Displays multiple levels with one normal, one active but offline, and one Welsh station with no values', async () => {
    ...
    Code.expect(response.payload).to.contain('<p>The River Pinn level upstream at Eastcote Road was <span data-item-value>0.35</span> metres. Property flooding is possible when it goes above 1.40 metres.')
    Code.expect(response.payload).to.contain('<p>The River Pinn level upstream at Avenue Road is currently unavailable.</p>')
    Code.expect(response.payload).to.not.contain('<p>The River Welsh station level upstream is currently unavailable.</p>')
    Code.expect(response.payload).to.contain('<p class="defra-live__supplementary">These levels will update automatically</p>')
  })
  ...
  lab.test('Displays Welsh station with values', async () => {
    ...
    Code.expect(response.statusCode).to.equal(200)
    Code.expect(response.payload).to.contain('<h2 class="defra-live__title">Latest level</h2>')
    Code.expect(response.payload).to.contain('<p>The River Welsh level upstream at Welsh Station was <span data-item-value>0.35</span> metres. Property flooding is possible when it goes above 1.40 metres.\n        \n      </p>')
  })
})
