...
lab.experiment('Target-area tests', () => {
  ...
  lab.test('GET target-area 011WAFDW ea Twitter handle is showing on end of situation', async () => {
    const floodService = require('../../server/services/flood')
    ...
    Code.expect(response.payload).to.contain('<p>Flood alert area: The Upper Derwent from Seathwaite to Derwent Water.</p>\n')
    Code.expect(response.payload).to.contain('<p>Test Situation in target area. Follow <a class="govuk-link" href="https://twitter.com/EnvAgencyNW">@EnvAgencyNW</a>')
  })
})
