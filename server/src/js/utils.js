...
window.flood.utils = {
  ...
  setGTagAnalyticsCookies: () => {
    const gtmAccId = window.flood && window.flood.config ? window.flood.config.gtmAccId : null
    ...
    script.innerHTML = `(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','${gtmAccId}');`

    const noscript = document.createElement('noscript')
    const iframe = document.createElement('iframe')
    iframe.setAttribute('src', `https://www.googletagmanager.com/ns.html?id=${gtmAccId}`)
  ...
  disableGoogleAnalytics: () => {
    // No-op for GTM: analytics is controlled by not loading the GTM snippet
  },
