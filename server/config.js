...
const schema = joi.object({
  ...
  azureMapsKey: joi.string().required(),
  azureMapsUrl: joi.string().uri().required(),
  ...
  fakeLocationCall: joi.boolean().default(false),
  floodFisUrl: joi.string().default(`http://localhost:${defaultPort}`),
  allowInsecureTls: joi.boolean().default(false),
...
const config = {
  ...
  azureMapsKey: process.env.FLOOD_APP_AZURE_MAPS_KEY,
  azureMapsUrl: process.env.FLOOD_APP_AZURE_MAPS_URL,
  ...
  fakeLocationCall: process.env.FLOOD_APP_FAKE_LOCATION_CALL,
  floodFisUrl: process.env.FLOOD_APP_FIS_URL,
  allowInsecureTls: process.env.FLOOD_APP_ALLOW_INSECURE_TLS,
